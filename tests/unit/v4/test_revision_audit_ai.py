from __future__ import annotations

import pytest

from caselinker.v4_contracts import ContractError, validate_instance

VALID_AUDIT = {
    "schema_version": "1.0",
    "contract_kind": "audit_event",
    "audit_event_id": "aud_example01",
    "actor_id": "prin_example01",
    "authority": "declared_binding",
    "reason_code": "accepted_bundle",
    "occurred_at": "2026-01-08T00:00:00Z",
    "prior_state": "extracted",
    "new_state": "resolved",
    "governing_input_ids": ["asrt_example01"],
}

VALID_AI = {
    "schema_version": "1.0",
    "contract_kind": "ai_execution",
    "execution_id": "aiex_example01",
    "provider": "fixture",
    "model_identity": "fixture-unspecified",
    "model_version": "0",
    "parameters_digest": "c" * 64,
    "execution_environment": "test",
    "prompt_identity": "prm_example01",
    "allowed_input_digest": "b" * 64,
    "tool_calls": [],
    "structured_output_valid": True,
    "policy_decision": "bounded_proposal",
    "egress_class": "none",
    "reproducibility_limitation": "fixture_only",
    "reviewer_disposition": "pending",
    "disposition": "proposed",
}


def test_audit_event_is_accepted() -> None:
    validate_instance("audit-event-v1", VALID_AUDIT)


def test_audit_event_rejects_source_passage() -> None:
    bad = {**VALID_AUDIT, "source_passage": "must not be stored"}
    with pytest.raises(ContractError, match="unknown field"):
        validate_instance("audit-event-v1", bad)


def test_complete_ai_execution_is_accepted() -> None:
    validate_instance("ai-execution-v1", VALID_AI)


def test_ai_self_review_is_rejected() -> None:
    bad = {**VALID_AI, "reviewer_disposition": "self_approved"}
    with pytest.raises(ContractError, match="self-review"):
        validate_instance("ai-execution-v1", bad)


def test_ai_still_cannot_publish() -> None:
    bad = {**VALID_AI, "disposition": "published"}
    with pytest.raises(ContractError, match="AI execution cannot publish"):
        validate_instance("ai-execution-v1", bad)
