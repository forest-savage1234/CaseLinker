...
SUT_REVISION = "w2e3-disclosure-v2"
...
def _decision_identity(decision: Mapping[str, object] | None) -> dict[str, object] | None:
    if decision is None:
        return None
    return {field: copy.deepcopy(decision.get(field)) for field in _DECISION_IDENTITY_FIELDS}
        ...
        results.append(
            {
                ...
                "outputs": outputs,
                "run_id": run_id,
                "subject_field_names": sorted(record),
            }
        )

    return {
        "schema_version": "w2e3-sut-output-v2",
